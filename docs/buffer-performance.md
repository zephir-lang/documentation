...
Where the [contiguous buffer](buffer.md) actually pays off, how to measure it, and what the numbers looked like when it was built.

File paths on this page are relative to the [zephir](https://github.com/zephir-lang/zephir) repository.
...
- [Contiguous buffer](buffer.md) — the feature itself, including why `foreach` is not a fast path.
- [Buffer internals](buffer-internals.md) — the six fast-path sites and the raw-pointer C API.
